...
// [[UIApplication sharedApplication] cancelAllLocalNotifications];
   ...
   // NSLog(@"response 1 %@",resultResponse1);
...
int i;
    ...
    for (id anUpdate in REMINDERitems1)
    {
     ...
     // NSLog(@"remaindertype:%@",arrayListType);
//NSLog(@"remaindername:%@",arrayListDate);
        
        
        
        
        NSString *dateString1 =[(NSDictionary*)anUpdate objectForKey:@"reminderdate"];
          // NSLog(@"datestring:%@",dateString1);
        NSDateFormatter *dateFormatter1 = [[NSDateFormatter alloc] init];
        
        dateFormatter1.locale = [[NSLocale alloc] initWithLocaleIdentifier:@"EN"];
      ...
      //  NSLog(@"datefromstring:%@",dateFromString1);
      ...
      localNotif = [[UILocalNotification alloc] init];
        ...
        if (localNotif == nil)
            return;
        localNotif.fireDate = dateFromString1;
   ...
   /*  NSMutableArray *SheduleArray=[[NSMutableArray alloc] initWithArray:[[UIApplication sharedApplication]scheduledLocalNotifications]];
             for(int s=0;s<[SheduleArray count];s++){
               ...
               int getId=[[Not.userInfo valueForKey:@"userid"] intValue];
 ...
 int length=[notificationArray1 count];
    for (i=0;i<length;i++)
    {
        localNotif=[notificationArray1 objectAtIndex:i ];
        NSString* idnumber=[localNotif.userInfo valueForKey:@"userid"];
       // NSLog(@"reminder id %@ ",[localNotif.userInfo valueForKey:@"userid"]);
        if([UserId isEqual:idnumber])
        {
           // NSLog(@"equal");
            [[UIApplication sharedApplication] scheduleLocalNotification:localNotif];
 ...
 // NSLog(@"final notifications %@",[[UIApplication sharedApplication]scheduledLocalNotifications]);
  ...
  // NSLog(@"RESULT ASSESS =%@",patQuestion);
    ...
    NSMutableArray*tempfromdate=[[NSMutableArray alloc]init];
NSMutableArray*temptodate=[[NSMutableArray alloc]init];
 ...
 // NSLog(@"NOTES:%@",[arrayList1 objectForKey:@"medicinenotes"]);
       //  NSLog(@"FROMD:%@",[arrayList1 objectForKey:@"fromd"]);
        // NSLog(@"TODATE:%@",[arrayList1 objectForKey:@"tod"]);
       // NSLog(@"Type:%@",[arrayList1 objectForKey:@"type"]);
        
        [tempM addObject:[arrayList1 objectForKey:@"medicinename"]];
            [tempD addObject:[arrayList1 objectForKey:@"direction"]];
        [tempNotes addObject:[arrayList1 objectForKey:@"medicinenotes"]];
          [tempfromdate addObject:[arrayList1 objectForKey:@"fromd"]];
        [temptodate addObject:[arrayList1 objectForKey:@"tod"]];
        [temptype addObject:[arrayList1 objectForKey:@"type"]];
    ...
    if([petArray count]==0)
    {
        for(int j=0;j<[tempM count];j++)
        {
            ...
            [recordDict setObject:[tempNotes objectAtIndex:j] forKey:@"notes"];
            [recordDict setObject:[tempfromdate objectAtIndex:j] forKey:@"fromd"];
            [recordDict setObject:[temptodate objectAtIndex:j] forKey:@"tod"];
           [recordDict setObject:[temptype objectAtIndex:j] forKey:@"type"];
            ...
            for(int j=0;j<[petArray count];j++)
            {
                ...
                if([s isEqualToString:s1])
                {
                    ...
                    if(xx==[petArray count])
                    {
                        ...
                        [recordDict setObject:[tempNotes objectAtIndex:i] forKey:@"notes"];
                        [recordDict setObject:[tempfromdate objectAtIndex:i] forKey:@"fromd"];
                        [recordDict setObject:[temptodate objectAtIndex:i] forKey:@"tod"];

                        [recordDict setObject:[temptype objectAtIndex:i] forKey:@"type"];
...
- (void)viewWillAppear:(BOOL)animated
{
   ...
   int syncType=[[NSUserDefaults standardUserDefaults]integerForKey:@"syncType"];
    ...
    {
                NSLog(@"automatic");
        self.timer = [NSTimer scheduledTimerWithTimeInterval:900
                                                      target:self
  ...
  //NSLog(@"local notifications in view will appear %@",reminderarray);
...
// NSLog(@"self.petarray in welcome view will appear:%@",self.petArray);
 NSMutableArray*a=[[NSMutableArray alloc]init];
    ...
    {
       // NSLog(@"yes");
        NSString*s1= [[self.petArray objectAtIndex:j] objectForKey:@"notes"];
        NSString *UserId = [[NSUserDefaults standardUserDefaults] objectForKey:@"loginid"];
...
//NSLog(@"petarray in view will appear %@",petArray);
...
-(NSString *)HttpPostEntityFirst:(NSString*)firstEntity ForValue1:(NSString*)value1  EntityThird:(NSString*)thirdEntity ForValue3:(NSString*)value3
{
    ...
    HUD.labelText = @"Feteching Appointments...";
